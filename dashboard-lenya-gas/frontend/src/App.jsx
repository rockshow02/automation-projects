...
const TABS = ["Dashboard", "Transaksi", "Hutang", "Penebusan", "Input"];
...
export default function App() {
  ...
  const [tebusForm, setTebusForm] = useState({
    tanggal_tebus: new Date().toISOString().split("T")[0],
    jumlah_tabung: "",
    harga_per_tabung: 16000,
    tanggal_datang: new Date(Date.now() + 86400000).toISOString().split("T")[0],
    catatan: ""
  });

  const load = async () => {
    const [l, m, h, t, p] = await Promise.all([
    ...
    setLaporan(l); setMingguan(m); setHutang(h); setTransaksi(t); setPenebusan(p);
  };
  ...
  const submitTebus = async (e) => {
    e.preventDefault();
    const payload = { ...tebusForm, total_modal: tebusForm.jumlah_tabung * tebusForm.harga_per_tabung };
    await fetch(`${API}/penebusan`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    showMsg("✅ Penebusan tersimpan! Notif Telegram terkirim.");
  ...
  const konfirmasiDatang = async (id) => {
    ...
    showMsg("✅ Gas dikonfirmasi datang! Notif Telegram terkirim.");
    load();
  };
  ...
  const statusColor = (s) => s === "Lunas" || s === "Sudah Datang" ? "#00C896" : s === "Hutang" || s === "Menunggu" ? "#F4A261" : "#FF6B6B";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0F1A", color: "#E8EDF5", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ background: "#0F1624", padding: "0 24px", display: "flex", gap: 4, borderBottom: "1px solid #1E2A3D", overflowX: "auto" }}>
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: "12px 18px", background: "none", border: "none", color: tab === t ? "#00C896" : "#5A7A9A", fontFamily: "'DM Sans', sans-serif", fontWeight: 500, fontSize: 14, cursor: "pointer", borderBottom: tab === t ? "2px solid #00C896" : "2px solid transparent", whiteSpace: "nowrap" }}>{t}</button>
        ))}
      </div>
      ...
      <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 20 }}>
              {[
                ...
                <div key={c.label} style={{ background: "#0F1624", border: "1px solid #1E2A3D", borderRadius: 14, padding: "16px 18px" }}>
                  ...
                  <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 16, color: c.accent }}>{c.value}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
              <div style={{ background: "#0F1624", border: "1px solid #1E2A3D", borderRadius: 14, padding: 18 }}>
                ...
                {[
                  ["Modal Penjualan", laporan.modal, "#5A7A9A"],
                  ["Modal Keluar (Tebus)", laporan.modal_keluar, "#FF6B6B"],
                  ...
                  ["Saldo Kas", laporan.saldo_kas, (laporan.saldo_kas || 0) >= 0 ? "#00C896" : "#FF6B6B"],
                ].map(([k, v, c]) => (
              ...
              <div style={{ background: "#0F1624", border: "1px solid #1E2A3D", borderRadius: 14, padding: 18 }}>
                ...
                {[
                  ["Stok Awal", `${laporan.stok_awal} tabung`, "#fff"],
                  ["Terjual", `${laporan.total_tabung} tabung`, "#F4A261"],
                  ["Sisa Stok", `${laporan.sisa_stok} tabung`, laporan.sisa_stok <= 10 ? "#FF6B6B" : "#00C896"],
                ].map(([k, v, c]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1E2A3D" }}>
                    <span style={{ fontSize: 13, color: "#8A9BAE" }}>{k}</span>
                ...
                {[
                  ["Sudah Dibayar", `Rp ${fmt(laporan.sudah_dibayar)}`, "#00C896"],
                  ["Belum Dibayar", `Rp ${fmt(laporan.belum_dibayar)}`, "#FF6B6B"],
                ].map(([k, v, c]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1E2A3D" }}>
                    <span style={{ fontSize: 13, color: "#8A9BAE" }}>{k}</span>
                    <span style={{ fontSize: 13, fontWeight: 600, color: c }}>{v}</span>
                  ...
                  {transaksi.map(t => (
                    <tr key={t.id} style={{ borderTop: "1px solid #1E2A3D" }}>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <span style={{ background: statusColor(t.status_bayar) + "22", color: statusColor(t.status_bayar), padding: "3px 10px", borderRadius: 99, fontSize: 12 }}>{t.status_bayar}</span>
                      </td>
                      <td style={{ padding: "12px 16px", fontSize: 12, color: "#5A7A9A" }}>{t.catatan || "-"}</td>
                      <td style={{ padding: "12px 16px" }}>
                        <button onClick={() => setStruk(t)} style={{ background: "#1E2A3D", color: "#E8EDF5", border: "1px solid #2A3F55", padding: "5px 12px", borderRadius: 8, fontSize: 12, cursor: "pointer" }}>🧾</button>
                      </td>
                    </tr>
                  ))}
                ...
                <tbody>
                  {hutang.map(h => (
                    <tr key={h.id} style={{ borderTop: "1px solid #1E2A3D" }}>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <button onClick={() => lunasin(h.id, h.total)} style={{ background: "#00C89622", color: "#00C896", border: "1px solid #00C89644", padding: "5px 12px", borderRadius: 8, fontSize: 12, cursor: "pointer" }}>Lunas</button>
                      </td>
                    </tr>
                  ))}
        ...
        {tab === "Penebusan" && (
          <div>
            <div style={{ background: "#0F1624", border: "1px solid #1E2A3D", borderRadius: 14, overflow: "hidden", marginBottom: 16 }}>
                  ...
                  <tbody>
                    {penebusan.map(p => (
                      <tr key={p.id} style={{ borderTop: "1px solid #1E2A3D" }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{ background: statusColor(p.status) + "22", color: statusColor(p.status), padding: "3px 10px", borderRadius: 99, fontSize: 12 }}>{p.status}</span>
                        </td>
                        <td style={{ padding: "12px 16px" }}>
                          {p.status === 'Menunggu' && (
                            <button onClick={() => konfirmasiDatang(p.id)} style={{ background: "#00C89622", color: "#00C896", border: "1px solid #00C89644", padding: "5px 12px", borderRadius: 8, fontSize: 12, cursor: "pointer" }}>✅ Gas Datang</button>
                          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={{ background: "#0F1624", border: "1px solid #1E2A3D", borderRadius: 14, padding: 20 }}>
              ...
              <form onSubmit={submitStok}>
                ...
                <button type="submit" style={{ width: "100%", background: "#00C896", border: "none", borderRadius: 10, padding: "12px", color: "#0A0F1A", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Simpan Stok</button>
              </form>
            </div>
            ...
            <div style={{ background: "#0F1624", border: "1px solid #1E2A3D", borderRadius: 14, padding: 20 }}>
              ...
              <form onSubmit={submitPenjualan}>
                ...
                <button type="submit" style={{ width: "100%", background: "#00C896", border: "none", borderRadius: 10, padding: "12px", color: "#0A0F1A", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Simpan Transaksi</button>
              </form>
            </div>

            <div style={{ background: "#0F1624", border: "1px solid #FF6B6B44", borderRadius: 14, padding: 20, gridColumn: "1/-1" }}>
              <div style={{ fontWeight: 600, color: "#fff", marginBottom: 4 }}>💸 Input Penebusan ke Agen</div>
              <div style={{ fontSize: 12, color: "#5A7A9A", marginBottom: 16 }}>Input H-1 sebelum gas datang. Notif otomatis ke Telegram.</div>
              <form onSubmit={submitTebus}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 13, color: "#8A9BAE", marginBottom: 6 }}>Tanggal Tebus</label>
                    <input type="date" value={tebusForm.tanggal_tebus} onChange={e => setTebusForm({ ...tebusForm, tanggal_tebus: e.target.value })} style={inputStyle} required />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 13, color: "#8A9BAE", marginBottom: 6 }}>Jumlah Tabung</label>
                    <input type="number" placeholder="contoh: 50" value={tebusForm.jumlah_tabung} onChange={e => setTebusForm({ ...tebusForm, jumlah_tabung: e.target.value })} style={inputStyle} required />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 13, color: "#8A9BAE", marginBottom: 6 }}>Tanggal Gas Datang</label>
                    <input type="date" value={tebusForm.tanggal_datang} onChange={e => setTebusForm({ ...tebusForm, tanggal_datang: e.target.value })} style={inputStyle} required />
                  </div>
                </div>
                ...
                <div style={{ marginTop: 12 }}>
                  <label style={{ display: "block", fontSize: 13, color: "#8A9BAE", marginBottom: 6 }}>Catatan (opsional)</label>
                ...
                <button type="submit" style={{ marginTop: 14, width: "100%", background: "#FF6B6B", border: "none", borderRadius: 10, padding: "12px", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Simpan Penebusan</button>
              </form>
            </div>
          </div>
        )}
